    ...
    <section className="w-full flex xl:flex-row flex-col justify-center min-h-screen gap-10 max-container">
        ...
        <div className="flex justify-start items-start flex-wrap w-full mt-20 gap-16">
          {statistics.map((stat) => (
            <div key={stat.label}>
              ...
              <p className="leading-7 font-motserrat text-slate-gray">
                {stat.label}
              </p>
            </div>
          ))}
      ...
      <div className="relative flex-1 flex justify-center items-center xl:min-h-screen max-xl:py-40 bg-primary bg-hero bg-cover bg-center">
        ...
        <div className="flex sm:gap-6 gap-4 absolute -bottom-[5%] sm:left-[10%] max-sm:px-6">
          {shoes.map((shoe) => (
            <ShoeCard
              key={shoe.id} // Ensure each shoe has a unique key
              imgURL={shoe}
              changeBigShoeImage={setBigShoeImg}
              bigShoeImg={bigShoeImg} // Pass the current big shoe image
            />
          ))}
        </div>
      </div>
